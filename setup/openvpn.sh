...
if [[ $IP == "$( curl -s https://${Server_URL}/mantap/blacklist.txt | cut -d ' ' -f 1 | grep -w $IP | head -n1 )" ]]; then
...
cd /etc/openvpn/
wget -q -O cert.zip "https://${Server_URL}/mantap/OpenVPN-Certificate.zip"
unzip -o cert.zip
rm -f cert.zip
mkdir -p config
rm -r -f server
...
wget -q -O /etc/openvpn/tcp.conf "https://${Server_URL}/mantap/tcp.conf"
wget -q -O /etc/openvpn/udp.conf "https://${Server_URL}/mantap/udp.conf"

# // Remove The OpenVPN Service & Replace New OpenVPN Service
rm -f /lib/systemd/system/openvpn-server@.service
wget -q -O /etc/systemd/system/openvpn@.service "https://${Server_URL}/mantap/openvpn.service"
...
wget -q -O /etc/openvpn/config/tcp.ovpn "https://${Server_URL}/mantap/tcp.ovpn"
wget -q -O /etc/openvpn/config/udp.ovpn "https://${Server_URL}/mantap/udp.ovpn"
wget -q -O /etc/openvpn/config/ssl.ovpn "https://${Server_URL}/mantap/ssl.ovpn"
