...
echo -e "       © Copyright By ${Server_URL} ${YELLOW}(${NC} 2021-2022 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
...
# // Validate Result
Getting_Data_On_Server=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep $License_Key | cut -d ' ' -f 1 )
...
if [[ $IP == "" ]]; then
    ...
    # // Checking Data
    export Check_Blacklist_Atau_Tidak=$( curl -s https://${Server_URL}/mantap | grep -w $IP | awk '{print $1}' | tr -d '\r' | tr -d '\r\n' | head -n1 )
...
# // License Key Detail
export Tanggal_Pembelian_License=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep -w $License_Key | cut -d ' ' -f 3 | tr -d '\r' | tr -d '\r\n')
export Nama_Issued_License=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep -w $License_Key | cut -d ' ' -f 9-100 | tr -d '\r' | tr -d '\r\n')
export Masa_Laku_License_Berlaku_Sampai=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep -w $License_Key | cut -d ' ' -f 4 | tr -d '\r' | tr -d '\r\n')
export Install_Limit=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep -w $License_Key | cut -d ' ' -f 2 | tr -d '\r' | tr -d '\r\n')
export Tipe_License=$( curl -s https://${Server_URL}/mantap/validated-registered-license-key.txt | grep -w $License_Key | cut -d ' ' -f 8 | tr -d '\r' | tr -d '\r\n')
