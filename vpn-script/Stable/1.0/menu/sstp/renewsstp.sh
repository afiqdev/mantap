...
# // Exporting URL Host
export Server_URL="raw.githubusercontent.com/geovpn/auto/main"
...
CLIENT_001=$( grep -c -E "^#DEPAN " "/etc/GeoVPN/SSTP-Server/client.db" )
echo "    =================================================="
echo "               LIST SSTP CLIENT ON THIS VPS"
echo "    =================================================="
grep -e "^#DEPAN " "/etc/GeoVPN/SSTP-Server/client.db" | cut -d ' ' -f 2-8 | nl -s ') '
...
user=$( grep -E "^#DEPAN " "/etc/GeoVPN/SSTP-Server/client.db" | cut -d ' ' -f 4 | sed -n "${CLIENT_002}"p)
exp=$( grep -E "^#DEPAN " "/etc/GeoVPN/SSTP-Server/client.db" | cut -d ' ' -f 8 | sed -n "${CLIENT_002}"p)
...
sed -i "s/#DEPAN Username : $user | Expired : $exp/#DEPAN Username : $user | Expired : $exp4/g" /etc/GeoVPN/SSTP-Server/client.db
sed -i "s/#BELAKANG Username : $user | Expired : $exp/#BELAKANG Username : $user | Expired : $exp4/g" /etc/GeoVPN/SSTP-Server/client.db
